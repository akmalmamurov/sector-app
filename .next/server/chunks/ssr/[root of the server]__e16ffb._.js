...
const UPDATE_ME = "/user/auth/send-otp";
...
request.interceptors.response.use((response)=>response, (error)=>{
    // if we got a response and it's a 401 with our specific message...
    if (error.response?.status === 401 && error.response.data?.message === 'Invalid or expired token') {
        // 1. clear auth state & remove token
        useStore.getState().logOut(); // Zustand’s getState() lets you call actions outside React :contentReference[oaicite:1]{index=1}
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__e16ffb._.js.map